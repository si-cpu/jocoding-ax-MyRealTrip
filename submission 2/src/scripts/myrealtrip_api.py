...
def discovery_detail_policy(item: dict[str, Any]) -> str:
    """Route expensive detail calls by product form."""
    form = item.get("purchaseForm") or classify_purchase_form(item)
    if form == "TOUR":
        return "DETAIL_FOR_DISCOVERY"
    if form == "PASS":
        return "DETAIL_ON_SELECTION"
    return "TITLE_FOR_DISCOVERY"
...
def partition_tna_items(
    ...
    for item in items:
        ...
        if reason:
            ...
            enriched["detailPolicy"] = discovery_detail_policy(enriched)
...
def category_is_discovery_eligible(category: dict[str, str]) -> bool:
    """Skip aggregate and pure-utility shelves while retaining transport discovery."""
    text = normalize_text(f"{category.get('name', '')} {category.get('value', '')}")
    if text in {"전체all", "allall"} or category.get("value") == "all":
        return False
    excluded = ("유심", "와이파이", "usimwifi", "여행용품", "travelgoods", "여행편의", "convenience")
    return not any(normalize_text(token) in text for token in excluded)
...
def collect_category_shelves(
    ...
    categories = [
        ...
        if category_is_discovery_eligible(category)
    ]
    ...
    return {
        ...
        "coverage": {
            "categoriesReturned": len(returned_categories),
            "categoriesCollected": len(shelves),
            "categoriesSkipped": len(returned_categories) - len(categories),
