...
Use evidence according to the product form.

Admission/ticket discovery:

1. search `itemName` or detail `title`
...
Pass access check after selection:

1. `included`
2. detail `description`
3. explicit option or usage information

Treat search `description` such as `오사카 ∙ 투어` as location/category metadata, not content evidence.
Treat `excluded` and other explicit non-inclusion fields as negative evidence only. A named attraction in exclusions must never create or support a product link for that experience.

Do not use an admission-ticket description to create additional initial experiences not named by the ticket. Do not expand a pass into an initial attraction list; use it only as an access method for an experience discovered independently.

If a tour description is clipped with `...` or the MCP detail omits a structured itinerary, do not complete the missing place name from general knowledge. Names in an `excluded` field such as separately paid admission fees do not, by themselves, prove that the itinerary visits those places.
