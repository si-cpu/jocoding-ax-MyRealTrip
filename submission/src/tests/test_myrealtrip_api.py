...
SCRIPT = pathlib.Path(__file__).parents[1] / "scripts" / "myrealtrip_api.py"
...
class ContentCollectionTests(unittest.TestCase):
    ...
    def test_category_shelves_collect_without_popularity_sort(self):
        ...
        self.assertTrue(all(call.kwargs["max_pages"] == 0 for call in collect.call_args_list))

    def test_category_shelves_default_to_all_pages(self):
        with (
            mock.patch.object(
                api,
                "post",
                return_value={"data": {"categories": [{"name": "투어", "value": "tour"}]}},
            ),
            mock.patch.object(
                api,
                "collect_search",
            ...
            api.collect_category_shelves("오사카")
        self.assertEqual(0, collect.call_args.kwargs["max_pages"])
