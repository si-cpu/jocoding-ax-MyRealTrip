...
아이디어를 곧바로 PRD로 확장하지 않고 검증 가능한 원자 명제로 분해합니다. 각 명제를 `SUPPORTED`, `PARTIAL`, `CONTRADICTED`, `UNKNOWN`으로 판정하고, 전체 아이디어에는 `GO`, `REVISE`, `HOLD`, `DROP` 중 하나와 가장 작은 다음 검증 행동을 제안합니다.
